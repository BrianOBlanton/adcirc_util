...
Type=Type(2:end);
if ~any(strcmp(Type,{'221','222','223','224','pre','wnd','win'}))
...
while ~feof(fid)
   ...
   if Debug>0
      disp(sprintf('Scanning [%d x %d] "%s" values at time=%s',iLong,iLat,Type,datestr(ctime,0)))
   end
   
   u=fscanf(fid,'%f',[iLong,iLat]);  
   
   if any(strcmp(Type,{'222','224','wnd','win'}))
     if Debug>0
        disp(sprintf('Scanning [%d x %d] "%s" values at time=%s',iLong,iLat,Type,datestr(ctime,0)))
     end
   ...
   if mod(k-1,Stride)==0
      if Debug>1
         disp(sprintf('   Storing [%d x %d] "%s" values at time=%s',iLong,iLat,Type,datestr(ctime,0)))
      end
      j=j+1;
      D(j).time=ctime;
      D(j).iLat=iLat;
      ...
      if Debug>1
         disp(sprintf('   Min,Max u = %f %f',min(u(:)),max(u(:))))
      end
      if any(strcmp(Type,{'222','224','wnd','win'}))
         D(j).Win.v=v;
         if Debug>1
            disp(sprintf('   Min,Max v = %f %f',min(u(:)),max(u(:))))
         end
      end      
   end
   fgetl(fid);
