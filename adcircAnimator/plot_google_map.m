...
if nargout <= 1 % plot map
    ...
    caxis(axHandle,cax); % Preserve caxis that is sometimes changed by the call to image()
    ...
    while ~strcmpi(get(figHandle, 'Type'), 'figure')
        % Recursively search for parent figuapp.gmap_handlere in case axes are in a panel
