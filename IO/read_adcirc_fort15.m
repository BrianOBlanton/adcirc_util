...
l=fgetl(fid);
temp=strtok(l,'!'); 
f15.nbfr=str2num(strtrim(temp));
f15.openbndampphase=NaN*ones(nopen,f15.nbfr*2);
if f15.nbfr>0
   for i=1:f15.nbfr
      l=fgetl(fid);
      l=fgetl(fid);
      [a,b,c]=strread(l,'%f%f%f');
      f15.bcfreqs(i)=a;
   end
   for i=1:f15.nbfr
      l=fgetl(fid);
      for j=1:nopen
         ...
         a=c{1};
         b=c{2};
         f15.openbndampphase(j,2*i-1)=a;
         f15.openbndampphase(j,2*i)=b;
      end
   end
end
