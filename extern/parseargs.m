function [X,unused] = parseargs(X,varargin)
...
while remaining>=2
    ...
    if ~isempty(fieldind)
        ...
        % store unused pvs to return to caller
        iu=iu+1;
        unused{iu}=fieldname;
        % get match in varargin
        idx=find(strcmp(fieldname,varargin));
        iu=iu+1;
        unused{iu}=varargin{idx+1};
