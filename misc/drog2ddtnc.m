...
%                           in the velocity sequence (obviously).
%                           The time must be in MATLAB's datenum or 
%                           datetime format. Defaults= beginning and end of
%                           input velocity field.
%         dt              - integration time step; this need NOT
%                           match the velocity sequence step.  This needs to be
%                           a MATLAB datetime duration, e.g., duration(0,15,0) 
%                           for a 15 minute integration step size.  Default= 
%                           time interval in velicity field.
...
if ~all(ismember({'u-vel','v-vel','time'},V.variables))
    error('Needed variables are missing in the ncgeodataset object')
end
...
tnow=t1d;
iter=1;
disp(['Starting: [' int2str(iter) ' ' datestr(tnow,0) ']'])
xnew=xi;
ynew=yi;
...
while tnow<t2d
   ...
   [xnext,ynext,jnext]=track(TheGrid,jgood,xgood,ygood,V,time,tnow,dt);
   j(igood)=jnext;
   xnew(igood)=xnext;
   ynew(igood)=ynext;
   ...
   [ut,vt]=vel_interp(TheGrid,xnext,ynext,jnext,V,time,tnow);
   xx(:,iter)      =xnew;
   yy(:,iter)      =ynew;
   uu(igood,iter)  =ut;
   vv(igood,iter)  =vt;
...
%       end
      ...
      hdrog2=line(xnew,ynew,'LineStyle','none','Marker','o', ...
                'MarkerSize',4,'MarkerFaceColor','g','MarkerEdgeColor','k');
      istuck=xnew==xx(:,iter-1) & ynew==yy(:,iter-1);
      hdrogstuck=line(xnew(istuck),ynew(istuck),'LineStyle','none','Marker','o', ...
                'MarkerSize',4,'MarkerFaceColor','y','MarkerEdgeColor','k');
      hdrogdead=line(xnew(isnan(j)),ynew(isnan(j)),'LineStyle','none','Marker','o', ...
...
% Prepare for return
disp(['Ending: [' int2str(iter) ' ' datestr(tnow,0) ']'])
D.tt=tt;
if mdrog==1
...
function [xnew,ynew,jnew]=track(TheGrid,j,x,y,V,timevec,t,dt)
jnew=j;
dts=seconds(dt);
% Mid-Point (RK2) step
...
[uinter,vinter]=vel_interp(TheGrid,xinter,yinter,jinter,V,timevec,t+.5*dt);
uk2=dts*uinter;
vk2=dts*vinter;
xnew=x+uk2;
ynew=y+vk2;
...
function [u,v]=vel_interp(TheGrid,x,y,j,V,time,t)
